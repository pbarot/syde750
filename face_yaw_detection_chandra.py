...
#looping for number of faces...
    for det in dets:
        ...
        ctr_x=frame_center[1]
        ctr_x=float(ctr_x)/2
        x_dist2=-(r_face_ctr[0]-ctr_x)
        x_dist*=px_to_mm
        x_dist2*=px_to_mm
        depth=depth/10
        if depth>100:
            yaw_angle=math.degrees(math.atan(x_dist/depth))
            yaw_angle2=math.degrees(math.atan(x_dist2/depth))
        #print("Xdist:",x_dist)
            #print("FACE_POS_TYPE:",round(yaw_angle,1),"FRAME_CTR_TYPE:",round(yaw_angle2,1))
            print("DEPTH:",depth,"WIDTH:",round(x_dist2,1),"ANGLE:",round(yaw_angle2,1))
